  ...
  return (
    <section className="space-y-4 border border-zinc-200 bg-white p-4">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <label className="grid gap-1 text-sm"><span>Agent 角色</span><select className="rounded-md border border-zinc-300 px-3 py-2" value={role} onChange={(event) => setRole(event.target.value)}><option value="researcher">研究</option><option value="planner">规划</option><option value="retriever">旧版检索（兼容）</option><option value="verifier">旧版核验（兼容）</option></select></label>
